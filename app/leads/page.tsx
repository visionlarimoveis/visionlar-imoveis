...
const empty={cliente_id:'',imovel_id:'',corretor_id:'',interesse:'Compra' as any,status:'Lead' as any,orcamento:'',observacoes:''}
...
export default function LeadsPage(){
  ...
  function abrirEditar(l:any){
    setEditId(l.id)
    setForm({cliente_id:l.cliente_id||'',imovel_id:l.imovel_id||'',corretor_id:l.corretor_id||'',interesse:l.interesse||'Compra',status:l.status||'Lead',orcamento:String(l.orcamento||''),observacoes:l.observacoes||''})
    setModalOpen(true)
  }
  ...
  async function salvar(){
    ...
    setSaving(true)
    const payload={cliente_id:form.cliente_id,imovel_id:form.imovel_id||null,corretor_id:form.corretor_id||null,interesse:form.interesse,status:form.status,orcamento:form.orcamento?parseFloat(form.orcamento):null,observacoes:form.observacoes||null,data_contato:new Date().toISOString().split('T')[0]}
    let error
    if(editId){({error}=await supabase.from('leads').update(payload).eq('id',editId))}
    else{({error}=await supabase.from('leads').insert(payload))}
    if(error)showToast('❌ '+error.message)
    else{showToast(editId?'✅ Lead atualizado!':'🎯 Lead registrado!');setModalOpen(false);load()}
    setSaving(false)
  }
  ...
  return(
    <>
      <header className="bg-white border-b border-gray-200 px-6 h-[58px] flex items-center justify-between sticky top-0 z-40 shadow-sm">
        ...
        <button onClick={abrirNovo} className="btn-gold text-xs px-4 py-1.5 rounded-lg">+ Novo Lead</button>
      </header>

      <div className="bg-white border-b border-gray-200 px-5 py-3 flex gap-2 flex-wrap">
        {['','Lead','Negociando','Fechado','Perdido'].map(f=>(
          <button key={f} onClick={()=>setFiltro(f)}
            className={`px-3 py-1.5 rounded-full text-xs font-semibold border transition-all ${filtro===f?'bg-[#0D2137] text-white border-[#0D2137]':'border-gray-200 text-gray-500 hover:border-gray-400'}`}>
            {f||'Todos'}
          </button>
        ))}
      </div>
      ...
      <main className="p-5">
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          {filtered.length===0?<div className="text-center py-12 text-gray-400"><div className="text-3xl mb-2 opacity-40">🎯</div><p className="text-sm">Nenhum lead</p></div>:(
            <table className="w-full">
              ...
              <tbody>{filtered.map(l=>(
                <tr key={l.id} className="border-t border-gray-100 hover:bg-gray-50/50">
                  ...
                  <td className="px-4 py-3 text-xs text-gray-400 max-w-[130px] truncate">{l.imovel?.titulo||'—'}</td>
                  ...
                  <td className="px-4 py-3"><div className="flex gap-1.5 items-center">
                    ...
                    <button onClick={()=>abrirEditar(l)} className="bg-amber-50 text-amber-700 text-[10px] px-2 py-1 rounded-lg hover:bg-amber-100 border border-amber-200 font-bold">✏️</button>
                    <button onClick={()=>deletar(l.id)} className="bg-red-50 text-red-600 text-[10px] px-2 py-1 rounded-lg hover:bg-red-100 border border-red-100 font-bold">🗑</button>
                  </div></td>
                </tr>
              ))}</tbody>
            </table>
        ...
        <div className="fixed inset-0 bg-gray-900/70 z-50 flex items-center justify-center p-4 backdrop-blur-sm" onClick={e=>{if(e.target===e.currentTarget)setModalOpen(false)}}>
          <div className="bg-white rounded-2xl w-full max-w-xl max-h-[92vh] overflow-y-auto shadow-2xl">
            <div className="px-5 py-4 border-b border-gray-200 flex items-center justify-between sticky top-0 bg-white">
              <h2 className="text-sm font-bold text-gray-900">{editId?'✏️ Editar Lead':'🎯 Novo Lead'}</h2>
            ...
            <div className="p-5 grid grid-cols-2 gap-3">
              <div><label className="text-[11px] font-bold text-gray-700 block mb-1">Cliente<span className="text-red-500">*</span></label><select className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-amber-500 bg-white" value={form.cliente_id} onChange={inp('cliente_id')}><option value="">Selecione</option>{clientes.map(c=><option key={c.id} value={c.id}>{c.nome}</option>)}</select></div>
              ...
              <div><label className="text-[11px] font-bold text-gray-700 block mb-1">Imóvel de Interesse</label><select className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-amber-500 bg-white" value={form.imovel_id} onChange={inp('imovel_id')}><option value="">— Nenhum —</option>{imoveis.map(i=><option key={i.id} value={i.id}>{i.titulo.substring(0,38)}</option>)}</select></div>
              ...
              <div><label className="text-[11px] font-bold text-gray-700 block mb-1">Orçamento (R$)</label><input type="number" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-amber-500" value={form.orcamento} onChange={inp('orcamento')}/></div>
            ...
            <div className="px-5 py-3 border-t border-gray-200 flex gap-2 justify-end">
              ...
              <button onClick={salvar} disabled={saving} className="btn-gold text-xs px-5 py-2 rounded-lg disabled:opacity-60">{saving?'Salvando...':(editId?'💾 Salvar Alterações':'💾 Registrar')}</button>
            </div>
          </div>
        </div>
      )}
