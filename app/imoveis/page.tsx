...
const COMODIDADES_DISPONIVEIS = [
  'Acessibilidade para PNE','Aquecimento a gás','Ar condicionado',
  'Área de serviço','Banheiro social','Churrasqueira',
  'Closet','Cozinha','Cozinha americana',
  'Dep. empregada','Despensa','Elevador',
  'Espaço gourmet','Espera para split','Estar social',
  'Fechadura digital','Gás individual','Hidrômetro individual',
  'Home office','Interfone','Lareira',
  'Lavabo','Sacada','Sala de estar',
  'Sala de jantar','Varanda','Varanda gourmet',
  'Vista panorâmica','Piscina','Academia',
  'Salão de festas','Playground','Portaria 24h',
  'Câmeras de segurança','Gerador','Jardim',
]

const emptyForm = {
  codigo:'', titulo:'', tipo:'', finalidade:'Venda' as 'Venda'|'Aluguel',
...
export default function ImoveisPage(){
  ...
  function toggleComodidade(c:string){
    setForm(f=>({
      ...f,
      comodidades: f.comodidades.includes(c)
        ? f.comodidades.filter(x=>x!==c)
        : [...f.comodidades, c]
    }))
  }
  ...
  async function salvar(){
    ...
    const payload:any={
      ...
      status:form.status, destaque:form.destaque, corretor_id:form.corretor_id||null,
      comodidades:form.comodidades,
    }
    // Só inclui codigo se preenchido
    ...
    <>
      <header className="bg-white border-b border-gray-200 px-6 h-[58px] flex items-center justify-between sticky top-0 z-40 shadow-sm">
        ...
        <div className="flex gap-2">
          <select className="text-xs border border-gray-200 px-2 py-1.5 rounded-lg bg-white outline-none" value={view} onChange={e=>setView(e.target.value as any)}><option value="grid">Grid</option><option value="lista">Lista</option></select>
        ...
        view==='grid'?(
            ...
            {filtered.map((i,idx)=>(
              <div key={i.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-lg transition-all">
                <div className="relative h-44 overflow-hidden bg-gray-100 cursor-pointer" onClick={()=>setVerOpen(i)}>
                  <img src={i.foto_url||FOTO_DEFAULT} alt={i.titulo} className="w-full h-full object-cover hover:scale-105 transition-transform duration-300" onError={e=>{(e.target as any).src=FOTO_DEFAULT}}/>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            <table className="w-full">
              <thead><tr className="bg-gray-50">{['Código','Imóvel','Tipo','Finalidade','Preço','Local','Status','Ações'].map(h=><th key={h} className="text-left text-[10px] font-bold text-gray-400 uppercase tracking-wider px-4 py-2.5">{h}</th>)}</tr></thead>
              <tbody>{filtered.map(i=>(
                <tr key={i.id} className="border-t border-gray-100 hover:bg-gray-50/50">
                  ...
                  <td className="px-4 py-3 text-xs font-semibold text-gray-900 max-w-[180px] truncate">{i.titulo}</td>
            ...
            <div className="p-5 grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="text-[11px] font-bold text-gray-700 block mb-1">
                  Código do Imóvel
                  <span className="text-gray-400 font-normal ml-1">(aparece no WhatsApp)</span>
                </label>
                <input className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-amber-500 font-mono tracking-wider uppercase" placeholder="Ex: AP-001, CS-042" value={form.codigo} onChange={inp('codigo')}/>
              </div>
              <div>
                <label className="text-[11px] font-bold text-gray-700 block mb-1">Finalidade<span className="text-red-500">*</span></label>
                <select className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-amber-500 bg-white" value={form.finalidade} onChange={inp('finalidade')}><option>Venda</option><option>Aluguel</option></select>
              </div>
              <div className="col-span-2">
                <label className="text-[11px] font-bold text-gray-700 block mb-1">Título<span className="text-red-500">*</span></label>
                <input className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-amber-500" placeholder="Ex: Apartamento 3 dorms no Centro" value={form.titulo} onChange={inp('titulo')}/>
              </div>
              <div>
                <label className="text-[11px] font-bold text-gray-700 block mb-1">Tipo<span className="text-red-500">*</span></label>
                <select className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-amber-500 bg-white" value={form.tipo} onChange={inp('tipo')}><option value="">Selecione</option>{tipos.map(t=><option key={t.id}>{t.nome}</option>)}</select>
              </div>
              <div>
                <label className="text-[11px] font-bold text-gray-700 block mb-1">Preço (R$)<span className="text-red-500">*</span></label>
                <input type="number" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-amber-500" placeholder="350000" value={form.preco} onChange={inp('preco')}/>
              </div>
              <div>
                <label className="text-[11px] font-bold text-gray-700 block mb-1">Cidade<span className="text-red-500">*</span></label>
                <select className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-amber-500 bg-white" value={form.cidade_id} onChange={inp('cidade_id')}><option value="">Selecione</option>{cidades.map(c=><option key={c.id} value={c.id}>{c.nome} - {c.estado}</option>)}</select>
              </div>
              <div>
                <label className="text-[11px] font-bold text-gray-700 block mb-1">Bairro</label>
                <select className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-amber-500 bg-white" value={form.bairro_id} onChange={inp('bairro_id')}><option value="">Selecione</option>{bairros.map(b=><option key={b.id} value={b.id}>{b.nome}</option>)}</select>
              </div>
              <div>
                <label className="text-[11px] font-bold text-gray-700 block mb-1">Endereço</label>
                <input className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-amber-500" placeholder="Rua X, 123" value={form.endereco} onChange={inp('endereco')}/>
              </div>
              <div>
                <label className="text-[11px] font-bold text-gray-700 block mb-1">Área (m²)</label>
                <input type="number" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-amber-500" value={form.area} onChange={inp('area')}/>
              </div>
              <div>
                <label className="text-[11px] font-bold text-gray-700 block mb-1">Dormitórios</label>
                <input type="number" min="0" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-amber-500" value={form.dorms} onChange={inp('dorms')}/>
              </div>
              <div>
                <label className="text-[11px] font-bold text-gray-700 block mb-1">Suítes</label>
                <input type="number" min="0" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-amber-500" value={form.suites} onChange={inp('suites')}/>
              </div>
              <div>
                <label className="text-[11px] font-bold text-gray-700 block mb-1">Banheiros</label>
                <input type="number" min="0" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-amber-500" value={form.banhs} onChange={inp('banhs')}/>
              </div>
              <div>
                <label className="text-[11px] font-bold text-gray-700 block mb-1">Vagas Garagem</label>
                <input type="number" min="0" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-amber-500" value={form.vagas} onChange={inp('vagas')}/>
              </div>
              <div>
                <label className="text-[11px] font-bold text-gray-700 block mb-1">Condomínio (R$/mês)</label>
                <input type="number" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-amber-500" value={form.condominio} onChange={inp('condominio')}/>
              </div>
              <div>
                <label className="text-[11px] font-bold text-gray-700 block mb-1">Corretor</label>
                <select className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-amber-500 bg-white" value={form.corretor_id} onChange={inp('corretor_id')}><option value="">Sem corretor</option>{corretores.map(c=><option key={c.id} value={c.id}>{c.nome}</option>)}</select>
              </div>
              <div>
                <label className="text-[11px] font-bold text-gray-700 block mb-1">Status</label>
                <select className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-amber-500 bg-white" value={form.status} onChange={inp('status')}><option>Ativo</option><option>Inativo</option><option>Vendido</option><option>Alugado</option></select>
              </div>
              <div>
                <label className="text-[11px] font-bold text-gray-700 block mb-1">Destaque no site?</label>
                <select className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-amber-500 bg-white" value={form.destaque?'1':'0'} onChange={e=>setForm(f=>({...f,destaque:e.target.value==='1'}))}><option value="0">Não</option><option value="1">Sim</option></select>
              </div>
              <div className="col-span-2">
                <label className="text-[11px] font-bold text-gray-700 block mb-1">Descrição</label>
                <textarea className="w-full border border-gray-200 rounded-lg px-3 py-2 text-xs outline-none focus:border-amber-500 resize-y min-h-[70px]" value={form.descricao} onChange={inp('descricao')}/>
              </div>
              <div className="col-span-2">
              ...
              <div className="col-span-2">
                <label className="text-[11px] font-bold text-gray-700 block mb-2">
                  ...
                  <span className="text-gray-400 font-normal ml-1">({form.comodidades.length} selecionadas)</span>
                </label>
                <div className="border border-gray-200 rounded-xl p-3 max-h-56 overflow-y-auto bg-gray-50">
                  <div className="grid grid-cols-3 gap-1.5">
                    {COMODIDADES_DISPONIVEIS.map(c=>(
                      <label key={c} className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg cursor-pointer transition-colors text-[11px] ${form.comodidades.includes(c)?'bg-[#0D2137] text-white':'bg-white text-gray-700 hover:bg-gray-100 border border-gray-200'}`}>
                        <input type="checkbox" className="hidden" checked={form.comodidades.includes(c)} onChange={()=>toggleComodidade(c)}/>
                        <div className={`w-3.5 h-3.5 rounded border-2 flex items-center justify-center shrink-0 transition-colors ${form.comodidades.includes(c)?'border-white bg-white':'border-gray-300'}`}>
                          {form.comodidades.includes(c)&&<svg width="8" height="6" viewBox="0 0 8 6" fill="none"><path d="M1 3L3 5L7 1" stroke="#0D2137" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>}
                        </div>
                        {c}
                      </label>
                    ))}
      ...
      {verOpen&&(
        <div className="fixed inset-0 bg-gray-900/70 z-50 flex items-center justify-center p-4 backdrop-blur-sm" onClick={e=>{if(e.target===e.currentTarget)setVerOpen(null)}}>
          <div className="bg-white rounded-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto shadow-2xl">
            <img src={verOpen.foto_url||FOTO_DEFAULT} alt={verOpen.titulo} className="w-full h-52 object-cover rounded-t-2xl"/>
            <div className="p-5">
              {/* Código */}
              {verOpen.codigo&&(
                <div className="inline-block bg-[#0D2137] text-white text-[10px] font-bold px-3 py-1 rounded-full mb-3">
                  Código: {verOpen.codigo}
                </div>
              )}
              ...
              {/* Comodidades */}
              {verOpen.comodidades?.length>0&&(
                <div className="mb-4">
                  <h4 className="text-xs font-bold text-gray-700 mb-2">Comodidades</h4>
                  <div className="flex flex-wrap gap-1.5">
                    {verOpen.comodidades.map((c:string)=>(
                      <span key={c} className="text-[10px] bg-emerald-50 text-emerald-700 px-2.5 py-1 rounded-full border border-emerald-100 flex items-center gap-1">
